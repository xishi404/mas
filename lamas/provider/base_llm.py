...
from lamas.configs.llm_config import LLMConfig
from lamas.const import LLM_API_TIMEOUT, USE_CONFIG_TIMEOUT
from lamas.logs import logger
from lamas.schema import Message
from lamas.utils.common import log_and_reraise
from lamas.utils.cost_manager import CostManager, Costs
...
class BaseLLM(ABC):
    ...
    async def aask_with_usage(
        ...
        """
        Same as aask but returns (text, prompt_tokens, completion_tokens) tuple.
        This allows thread-safe per-problem token counting in parallel execution.
        """
        if system_msgs:
            message = self._system_msgs(system_msgs)
        else:
        ...
        rsp = await self._achat_completion(message, timeout=self.get_timeout(timeout))
        ...
        if hasattr(rsp, 'usage'):
            ...
            completion_tokens = rsp.usage.completion_tokens if hasattr(rsp.usage, 'completion_tokens') else rsp.usage.output_tokens
            prompt_tokens = rsp.usage.prompt_tokens if hasattr(rsp.usage, 'prompt_tokens') else getattr(rsp.usage, 'input_tokens', 0)
        elif isinstance(rsp, dict) and 'usage' in rsp:
            # Dict-style response
            completion_tokens = rsp['usage'].get('completion_tokens', rsp['usage'].get('output_tokens', 0))
            prompt_tokens = rsp['usage'].get('prompt_tokens', rsp['usage'].get('input_tokens', 0))
