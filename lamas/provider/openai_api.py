...
from lamas.configs.llm_config import LLMConfig, LLMType
from lamas.const import USE_CONFIG_TIMEOUT
from lamas.logs import log_llm_stream, logger
from lamas.provider.base_llm import BaseLLM
from lamas.provider.constant import GENERAL_FUNCTION_SCHEMA
from lamas.provider.llm_provider_registry import register_provider
from lamas.utils.common import CodeParser, decode_image, log_and_reraise
from lamas.utils.cost_manager import CostManager
from lamas.utils.exceptions import handle_exception
...
class OpenAILLM(BaseLLM):
    ...
    def _cons_kwargs(self, messages: list[dict], timeout=USE_CONFIG_TIMEOUT, **extra_kwargs) -> dict:
        ...
        # OpenRouter provider routing: pin to a specific provider (e.g., DeepInfra)
        if getattr(self.config, 'provider', None):
            kwargs["extra_body"] = {"provider": {"order": [self.config.provider]}}
        if extra_kwargs:
