import asyncio
import torch
import lamas.ext.lamas.scripts.optimized.MATH.train.template.prompt as prompt_custom
import lamas.ext.lamas.scripts.optimized.MATH.train.template.prompt_short as prompt_short
import lamas.ext.lamas.scripts.optimized.MATH.train.template.operator as operator
import lamas.ext.lamas.scripts.optimized.MATH.train.template.op_prompt_short as op_prompt_short
from lamas.ext.lamas.scripts.optimized.MATH.train.template.operator_registry import operator_mapping, operator_names
...
class MATHGraph:
    ...
    async def _dispatch_operator(self, task, dep_solution, task_outputs, problem):
        """MATH-specific operator dispatch for DAG execution (shared with test/graph.py)."""
        import time
        ...
        if op_name in ["Generate", "GenerateCoT"]:
            ...
            if not resolved_solutions:
                latency = time.time() - task_start
                return "skipped", {"type": "noop", "solution": dep_solution, "operator": op_name, "latency": latency, "iterations": 0, "cp_token_count": 0, "_raw_prompt_tokens": 0, "_raw_completion_tokens": 0}
            else:
