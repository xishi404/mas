import asyncio
import torch
import lamas.ext.lamas.scripts.optimized.MATH.test.template.prompt as prompt_custom
import lamas.ext.lamas.scripts.optimized.MATH.test.template.prompt_short as prompt_short
import lamas.ext.lamas.scripts.optimized.MATH.test.template.operator as operator
import lamas.ext.lamas.scripts.optimized.MATH.test.template.op_prompt_short as op_prompt_short
from lamas.ext.lamas.scripts.optimized.MATH.test.template.operator_registry import operator_mapping, operator_names
...
from lamas.ext.lamas.scripts.optimized.scheduler import schedule, GENERATORS
...
class MATHGraph:
    def __init__(
        ...
        if self.pruning_gate is not None:
            self.pruning_gate = self.pruning_gate.to(self.device)
        self.tracer = None

        # Override operator prompts with short versions when enabled
        if short_prompt:
            logger.info("Using SHORT prompts for MATH operators")
    ...
    def _log_dag_paths(self, plan, task_outputs, cp_finish, critical_path_chain, critical_path_latency, wall_clock_time):
        """Log DAG execution timeline grouped by independent paths with critical path highlighted."""
        critical_path_ids = set(critical_path_chain)

        # Build successor map for single-dep chains (predecessor -> next task)
        successor_map = {}
        for tid, task in plan.task_index.items():
            if task.depends_on_solution and not task.depends_on_solutions:
                pred = task.depends_on_solution
                if pred not in successor_map:
                    successor_map[pred] = tid

        # Generators have no dependencies
        generators = sorted([tid for tid, task in plan.task_index.items()
                              if not task.depends_on_solution and not task.depends_on_solutions])

        # Build each chain from a generator forward
        chains = []
        covered = set()
        for gen_id in generators:
            ...
            covered.add(gen_id)
            cur = gen_id
            while cur in successor_map:
                nxt = successor_map[cur]
                chain.append(nxt)
                covered.add(nxt)
                cur = nxt
            chains.append(chain)

        # Fan-in nodes (e.g. ScEnsemble with multiple inputs)
        fan_in_tasks = [tid for tid in (t.task_id for layer in plan.layers for t in layer)
                        if tid not in covered]

        logger.info(f"\n{'='*70}")
        logger.info("SCHEDULER DAG PATHS")
        logger.info(f"{'='*70}")
        for i, chain in enumerate(chains):
            parts = []
            for tid in chain:
                r = task_outputs[tid]
                cp_marker = " [CP]" if tid in critical_path_ids else ""
                start = r.get("_start_offset", 0.0)
                lat = r.get("latency", 0.0)
                parts.append(f"{tid}(+{start:.2f}s,{lat:.2f}s{cp_marker})")
            logger.info(f"  Path {i + 1}: {' → '.join(parts)}")
        for tid in fan_in_tasks:
            r = task_outputs[tid]
            cp_marker = " [CP]" if tid in critical_path_ids else ""
            start = r.get("_start_offset", 0.0)
            lat = r.get("latency", 0.0)
            n_deps = len(plan.task_index[tid].depends_on_solutions or [])
            logger.info(f"  Merge({n_deps} inputs): {tid}(+{start:.2f}s,{lat:.2f}s{cp_marker})")
        logger.info(f"\n  Critical path: {' → '.join(critical_path_chain)}")
        logger.info(f"  Critical path latency: {critical_path_latency:.3f}s | Wall-clock: {wall_clock_time:.3f}s")
        logger.info(f"{'='*70}\n")

    async def _dispatch_operator(self, task, dep_solution, task_outputs, problem):
        """MATH-specific operator dispatch for DAG execution."""
        import time
        ...
        if op_name in ["Generate", "GenerateCoT"]:
            result = await selected_operator(input=problem, instruction=self.prompt_module.DETAILED_SOLUTION_PROMPT, return_usage=True)
            ...
            result = await selected_operator(input=problem, instruction=self.prompt_module.GENERATE_SOLUTION_PROMPT, return_usage=True)
            latency = time.time() - task_start
            ...
            refined_solution = await self.custom(input=problem + f"\nCode output: {result['output']}", instruction=self.prompt_module.REFINE_ANSWER_PROMPT, return_usage=True)
            latency = time.time() - task_start
    ...
    async def _execute_scheduled(self, problem, plan, probs_layers, log_probs_layers, log_path):
        """Execute with fine-grained DAG dependency tracking via shared executor."""
        ...
        task_outputs, total_operator_time, gate_log_probs, pruning_triggered, _gate_state = await execute_dag(
        ...
        # Critical path
        cp_finish, critical_path_chain, critical_path_latency = compute_critical_path(plan, task_outputs)
        self._log_dag_paths(plan, task_outputs, cp_finish, critical_path_chain, critical_path_latency, total_operator_time)
        ...
        sched_log_probs_per_layer = rebuild_log_probs(plan, probs_layers, self.selection_operator_names, self.device)

        # Include gate log_probs in sum_log_prob for REINFORCE
        sum_log_prob = sum(log_probs_layers)
        if gate_log_probs:
            sum_log_prob = sum_log_prob + sum(gate_log_probs)

        return (solutions, current_solution, sum_log_prob, metrics["problem_prompt_tokens"],
                metrics["problem_completion_tokens"], total_operator_time, metrics["operator_latencies"],
                metrics["operator_iterations"], metrics["operator_names_per_layer"], metrics["operator_latencies_per_layer"],
                metrics["operator_token_counts_per_layer"], sched_log_probs_per_layer)
    ...
    async def __call__(self, problem: str, log_path: str = None):
        ...
        if self.use_scheduler:
            ...
            (solutions, current_solution, sum_log_prob, problem_prompt_tokens,
             problem_completion_tokens, total_operator_time, operator_latencies,
             operator_iterations, operator_names_per_layer, operator_latencies_per_layer,
             operator_token_counts_per_layer, log_probs_per_layer) = await self._execute_scheduled(
                problem, plan, probs_layers, log_probs_layers, log_path)

            # Jump to final ensemble / post-processing
            # (falls through to the code after the main loop)
        else:
                ...
                if self.parallel_execution:
                    ...
                    for op_name in selected_names:
                        ...
                        if op_name == "MultiGenerateCoT":
                            ...
                            problem_prompt_tokens += refined_solution.get('_usage_prompt_tokens', 0) if 'refined_solution' in locals() else 0
                            problem_completion_tokens += refined_tokens
        ...
        if len(solutions) > 1:
            ...
            problem_prompt_tokens += ensemble_result.get('_usage_prompt_tokens', 0)
            problem_completion_tokens += ensemble_tokens
        else:
            final_solution = current_solution
        ensemble_time = time.time() - post_process_start
        if self.tracer and ensemble_tokens > 0:
            self.tracer.log(layer=-1, operator="ScEnsemble_post", input=problem, output={"selected": final_solution, "num_solutions": len(solutions)}, latency=ensemble_time, tokens=ensemble_tokens)

        # Append post-processing as additional pseudo-layer for per-operator tracking
        if ensemble_tokens > 0:
            operator_names_per_layer.append(["ScEnsemble_post"])
            operator_token_counts_per_layer.append([ensemble_tokens])
            operator_latencies_per_layer.append([ensemble_time])
