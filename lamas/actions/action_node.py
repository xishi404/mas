...
from lamas.actions.action_outcls_registry import register_action_outcls
from lamas.const import USE_CONFIG_TIMEOUT
from lamas.llm import BaseLLM
from lamas.logs import logger
from lamas.provider.postprocess.llm_output_postprocess import llm_output_postprocess
from lamas.utils.common import OutputParser, general_after_log
from lamas.utils.human_interaction import HumanInteraction
...
"""
    ...
    async def code_fill(
        self, context: str, function_name: Optional[str] = None, timeout: int = USE_CONFIG_TIMEOUT, return_usage: bool = False
    ) -> Dict[str, str]:
        """
        ...
        prompt = context
        if return_usage:
            content, prompt_tokens, completion_tokens = await self.llm.aask_with_usage(prompt, timeout=timeout)
            extracted_code = sanitize(code=content, entrypoint=function_name)
            result = {field_name: extracted_code, "_usage_tokens": completion_tokens, "_usage_prompt_tokens": prompt_tokens}
        else:
    ...
    async def single_fill(self, context: str, images: Optional[Union[str, list[str]]] = None, return_usage: bool = False) -> Dict[str, Any]:
        field_name = self.get_field_name()
        prompt = context
        if return_usage:
            content, prompt_tokens, completion_tokens = await self.llm.aask_with_usage(prompt, images=images)
            result = {field_name: content, "_usage_tokens": completion_tokens, "_usage_prompt_tokens": prompt_tokens}
        else:
            content = await self.llm.aask(prompt, images=images)
            result = {field_name: content}
        return result

    async def xml_fill(self, context: str, images: Optional[Union[str, list[str]]] = None, return_usage: bool = False) -> Dict[str, Any]:
        """
        ...
        if return_usage:
            content, prompt_tokens, completion_tokens = await self.llm.aask_with_usage(context, images=images)
        else:
        ...
        if return_usage:
            extracted_data["_usage_tokens"] = completion_tokens
            extracted_data["_usage_prompt_tokens"] = prompt_tokens
    ...
    async def fill(
        ...
        if mode == FillMode.CODE_FILL.value:
            result = await self.code_fill(context, function_name, timeout, return_usage=return_usage)
            # Store usage tokens separately before passing to Pydantic (which will ignore unknown fields)
            usage_tokens = result.pop("_usage_tokens", 0) if return_usage else 0
            usage_prompt_tokens = result.pop("_usage_prompt_tokens", 0) if return_usage else 0
            ...
            result = await self.xml_fill(context, images=images, return_usage=return_usage)
            usage_tokens = result.pop("_usage_tokens", 0) if return_usage else 0
            usage_prompt_tokens = result.pop("_usage_prompt_tokens", 0) if return_usage else 0
            self.instruct_content = self.create_class()(**result)
            if return_usage:
            ...
            result = await self.single_fill(context, images=images, return_usage=return_usage)
            usage_tokens = result.pop("_usage_tokens", 0) if return_usage else 0
            usage_prompt_tokens = result.pop("_usage_prompt_tokens", 0) if return_usage else 0
            self.instruct_content = self.create_class()(**result)
            if return_usage:
